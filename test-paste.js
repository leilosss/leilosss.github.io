...
const CASES = [
  ...
  {
    // 真实账单导出(微信/支付宝的 CSV·XLSX·交易明细证明)一律"新的在前"。
    // 引擎曾按输入顺序算扣费间隔 → 负数间隔落在所有周期窗口之外 → 一笔都认不出,
    // 首末日期还会颠倒。这条用例把顺序钉死:同样的数据倒过来必须得出同样的结论。
    name: "新的在前(账单导出的真实顺序)",
    text: [
      "微信支付账单明细",
      "2026-08-10 商户消费", "Netflix 会员", "¥49.00",
    ...
    expect: (r) => {
      if (r.count !== 1) throw new Error("倒序账单应识别 1 个订阅,实得 " + r.count + " → " + r.names.join(" / "));
      if (r.annual !== 588) throw new Error("年化应 588(49×12),实得 " + r.annual);
    },
  },
  {
